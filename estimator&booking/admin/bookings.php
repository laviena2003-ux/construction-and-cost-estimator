...
// Fetch bookings with customer info and payment status
try {
    $bookings = $conn->query("
        SELECT b.*, c.name AS customer_name, c.email,
               (CASE WHEN p.id IS NOT NULL THEN 'Paid' ELSE 'Pending' END) AS display_status
        FROM bookings b
...
<title>Manage “Construction & Estimation” Bookings</title>
<style>
...
tr:hover { background: #b27c00(240, 238, 229); }

.status-paid { color: #2e7d32; font-weight: bold; }   /* green */
.status-pending { color: #b27c00; font-weight: bold; } /* orange */

.btn-delete { background: #c62828; padding: 6px 12px; border: none; border-radius: 5px; color: #fff; font-weight: bold; cursor: pointer; transition: 0.3s; text-decoration: none; display: inline-block; }
...
<div class="navbar">
    <div class="title">Construction Booking & Estimation</div>
    <div class="nav-links">
...
<div class="container">
    <h1>“Construction Booking & Estimation”</h1>
    <table>
        <tr>
            <th>ID</th>
            <th>Customer</th>
        ...
        <?php if ($bookings): ?>
            <?php foreach ($bookings as $b): ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($b['area_sqft'] ?? 'N/A') ?></td>
                    <td><?= $b['total_cost'] ? number_format($b['total_cost'],2) : 'N/A' ?></td>
                    ...
                    <td>
                        <a href="bookings.php?delete_id=<?= $b['id'] ?>" class="btn-delete">❌ Delete</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        <?php else: ?>
