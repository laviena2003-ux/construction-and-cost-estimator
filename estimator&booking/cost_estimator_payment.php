...
// Get booking_id from GET or session
$booking_id = $_GET['booking_id'] ?? $_SESSION['last_booking_id'] ?? null;
$customer_id = $_SESSION['customer_id'] ?? null;

if (!$booking_id || !$customer_id) {
    die("❌ Booking ID is missing or user session is invalid.");
}

// Fetch booking info and ensure it belongs to this customer
    ...
    if ($card_number && $card_holder && $expiry_date && $cvv) {
        try {
            ...
            $stmt = $conn->prepare("
                ...
                (booking_id, customer_id, card_number, card_holder, expiry_date, cvv, amount, payment_date, status)
                VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), 'Paid')
            ");
            $stmt->execute([$booking_id, $customer_id, $card_number, $card_holder, $expiry_date, $cvv, $amount]);
...
<head>
<meta charset="UTF-8">
<title>Cost Estimator Payment</title>
...
<div class="navbar">
    <div>Cost Estimator Payment</div>
    <div>
        <a href="index.php">Home</a>
        <a href="logout.php"><i class="fa fa-sign-out-alt"></i>Logout</a>
        <a href="cost_estimator.php">back</a>
    </div>
</div>
